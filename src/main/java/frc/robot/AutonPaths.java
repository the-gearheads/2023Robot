...
public class AutonPaths {
  ...
  public static CommandBase getTestPlacePath(Subsystems s) {
    return new SequentialCommandGroup(
        ...
        getCommandForPath("Inert_To_Start", true, Constants.AUTON.SLOW_CONSTRAINTS, s.swerve), getPlaceConeCommand(s),
        ...
        // Place it
        getGrabberOpenCommand(s.grabber), new WaitCommand(0.5), new SetArmPose(s.arm, ArmPose.INSIDE_ROBOT));
  }
  ...
  public static Command getPlaceConeCommand(Subsystems s) {
    return new SequentialCommandGroup(setWristModeCommand(s.wrist, WristStateType.ALT), new WaitCommand(0.5), // Wait for wrist to rotate before dropping cone
        getGrabberOpenCommand(s.grabber), new WaitCommand(0.5), // Wait for grabber and gravity to drop cone
        getGrabberCloseCommand(s.grabber), setWristModeCommand(s.wrist, WristStateType.DEFAULT), // Move wrist back
