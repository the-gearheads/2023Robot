...
public class SendableArmFeedforward implements Sendable {
   ...
   *
   * @param positionDeg The position (angle) setpoint. This angle should be measured from the horizontal (i.e. if the provided angle is 0, the arm should be parallel
   ...
   * @param positionDeg The angle of the arm. This angle should be measured from the horizontal (i.e. if the provided angle is 0, the arm should be parallel with the floor). If
  ...
  public double maxAchievableAcceleration(double maxVoltage, double positionDeg, double velocityDegPerSec) {
    double positionRad = Units.degreesToRadians(positionDeg);
    double velocityRadPerSec = Units.degreesToRadians(velocityDegPerSec);
    return (maxVoltage - ks * Math.signum(velocityRadPerSec) - Math.cos(positionRad) * kg - velocityRadPerSec * kv) / ka;
  }
